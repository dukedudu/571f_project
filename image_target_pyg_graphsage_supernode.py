...
def train_target_node2vec(args):
    ...
    while iter_num < max_iter:
        ...
        if args.cls_par > 0:
            ...
            # print(fea_lookup)
            # print(fea_lookup['57'])
            feat_from_node2vec = torch.from_numpy(fea_lookup[tar_idx_batch]).float().to(torch.device("cuda:0"))
            feat_loss = nn.MSELoss()
            feat_dist_loss = torch.mean(feat_loss(features_test, feat_from_node2vec))
...
def obtain_vec(loader, netF, netB, netC, args):
    ...
    # normalize p but not f
    all_output = (nn.Softmax(dim=1)(all_output).t() / torch.norm(all_fea, p=2, dim=1)).t()
    # all_fea = (all_fea.t() / torch.norm(all_fea, p=2, dim=1)).t()

    edge = all_output @ all_output.T  # edge graph
    # print(edge.size())
    ...
    node2vec = Node2Vec(graph, dimensions=256, walk_length=10, num_walks=200,
                        workers=8)  # Use temp_folder for big graphs
