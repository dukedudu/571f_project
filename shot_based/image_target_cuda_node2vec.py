...
def train_target_node2vec(args):
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 and args.cls_par > 0:
            netF.eval()
            netB.eval()
            node, edge, prob_lookup = obtain_vec(dset_loaders['test'], netF, netB, netC, args)
            mem_label = obtain_label(dset_loaders['test'], netF, netB, netC, args)
            mem_label = torch.from_numpy(mem_label).cuda()
            netF.train()
            netB.train()
        ...
        if args.enable_node2vec:
            ...
            feat_dist_loss = nn.CrossEntropyLoss()(softmax_out, prob_from_node2vec.softmax(dim=1))
        else:
...
def obtain_vec(loader, netF, netB, netC, args):
    ...
    # nx.set_node_attributes(graph, feature_dict, "feat")  # value and name are switched.
      nx.set_node_attributes(graph, feature_dict, "prob")  # value and name are switched.
    ...
    # node2vec = Node2Vec(graph, dimensions=256, walk_length=10, num_walks=200,
    #                     workers=4)  # Use temp_folder for big graphs
      node2vec = Node2Vec(graph, dimensions=K, temp_folder=args.graph_folder, walk_length=args.walk_length, num_walks=args.walk_length,
                        workers=4, p=0.5, q=1.5)  # Use temp_folder for big graphs
      model = node2vec.fit(window=10, min_count=1, batch_words=4)
      fea_lookup = model.wv
      print("generating node embedding complete")
    else:
      fea_lookup = None
    return all_fea, graph, fea_lookup
