...
def train_target_node2vec(args):
    ...
    while iter_num < max_iter:
        ...
        if args.cls_par > 0:  # batch level
            ...
            feat_dist_loss = torch.tensor(0.0)
        if args.ent:
...
def obtain_label(loader, netF, netB, netC, args):
    ...
    if args.distance == 'cosine':
        all_fea = torch.cat((all_fea, torch.ones(all_fea.size(0), 1)), 1)  # normalize
...
def obtain_vec(loader, netF, netB, netC, args):
    ...
    model = Node2Vec(torch.LongTensor(edge_index), embedding_dim=256, walk_length=30,
